...
const EditReservation = ({setPage, setLastPage, lastPage, reservation}) => {
    ...
    const editReservation = async () => {
        ...
        if(name_val && status_val && at_val && ui_val && un_val){
            //alert("going")
            setAction("Loading...")
            const url = `${getAPIBaseURL()}/v1/admin/reservation/edit`;
            ...
            const data = {name, status: statusCode, id: reservation._id};
    ...
    return(
        ...
        <div class="mx-5 md:mx-20 mt-10">
            ...
            <button onClick={() => {setPage(lastPage); setLastPage("Lits")}} type="button" class="mt-1 text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-full text-sm p-2.5 text-center inline-flex items-center mr-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                ...
                <h1 class="sm:text-3xl text-2xl mb-2 text-black">Edit reservation</h1>
            </div>
            <div class="my-5">
            ...
            <button onClick={() => editReservation()} type="submit" class="flex text-white bg-blue-700 px-6 py-2 mb-8">{action}</button>
        </div>
        </div>
